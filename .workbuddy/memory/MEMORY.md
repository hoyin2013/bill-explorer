...
- **金额列 = 数量×单价（2026-08-15 三次演进）**：① 最初是 `=E*F` 活公式，但 `loadSheet.cellToText` 优先返回公式串导致 Univer 重算成 0 → 编辑框显示 0。② 改成 `recomputeAmount` 写**静态数值** `setValue(q*p)`，并加「金额已是正数则不覆盖」守卫。③ 该守卫导致「插入/复制行后改数量单价、金额不重算」（即用户说的“公式不生效”）。**最终版（当晚）**：`recomputeAmount` 在 数量>0 且 单价>0 时**无条件 `setValue(q*p)` 覆盖金额**（不再判断金额旧值）；仅当 数量或单价 缺失(0/空) 才不动金额列——以便“无数量单价的整单总价行”能手填独立金额。这样插入新行/改数量单价/复制行，金额都自动生效、恒等于数量×单价。
- **金额读取铁律**：`loadSheet.cellToText`（src/main/excel-memo.ts）对公式单元格**优先返回 Excel 缓存的 `result`**（仅当无缓存才退回公式串），保证编辑框与 Excel 显示一致、不重算成 0。存盘金额走普通数值单元格；数量/单价/金额须数值化避免文本。OCR 金额留空、由 `SheetValueChanged` 监听自动算 `q*p`。
