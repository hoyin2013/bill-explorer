...
- **金额列 = 数量×单价 的「静态计算结果」（2026-08-15 改；当日下午又修正，见下）**：金额(列6,Excel=G) 在填 数量(列4,E)/单价(列5,F) 时由 `UniverSheet.recomputeAmount` 写入「数量×单价」的**静态数值**（`setValue(q*p)`），不再写 `=E*F` 活公式；金额列已有合理数值（用户手填）则不覆盖。`=E*F` 活公式曾导致「原文件金额有数据、编辑框却显示 0」——根因是 `loadSheet.cellToText` 优先返回公式串、Univer 按自身上下文重算（引用/文本相乘）得 0。
- **金额读取铁律（修正版）**：`loadSheet.cellToText`（src/main/excel-memo.ts）对公式单元格**优先返回 Excel 缓存的 `result`**（即 Excel 里显示的值），仅当无缓存结果才退回公式串。这样编辑框看到的值与 Excel 完全一致，金额有数据就显示数据、不再被重算成 0。**因此金额列已退化为静态数值，不再依赖 Univer 公式引擎**；存盘时金额走普通数值单元格（`{v:Number}`），数量/单价/金额仍须数值化避免文本。OCR 识别出金额则保留数值、否则留空等 `SheetValueChanged` 监听自动算 `q*p`。
