...
// 固定账单表头（8 列，顺序即默认列顺序）—— 这是写入与匹配的源头
const HEADER = [
  { key: 'date',   label: '日期' },
  { key: 'name',   label: '货品名称' },
  { key: 'unit',   label: '单位' },
...
function isEmptyRow(r: BillRecord): boolean {
  return !r.date && !r.name && !r.unit && !r.person && !r.remark && r.qty === 0 && r.price === 0
}
...
//   3. 文件有数据但未识别表头 → 按默认列序(1..8)写入，并提示
...
export async function appendRows(
  ...
  validRows.forEach((record, i) => {
    ...
    row.getCell(colMap.date).value = toExcelDate(record.date)
    row.getCell(colMap.date).numFmt = 'yyyy-mm-dd'
    row.getCell(colMap.name).value = record.name
    row.getCell(colMap.unit).value = record.unit
    row.getCell(colMap.qty).value = record.qty
    row.getCell(colMap.qty).numFmt = '#,##0.##'
    row.getCell(colMap.price).value = record.price
    row.getCell(colMap.price).numFmt = '#,##0.00'
    row.getCell(colMap.amount).value = record.amount
    row.getCell(colMap.amount).numFmt = '#,##0.00'
    row.getCell(colMap.person).value = record.person
    row.getCell(colMap.remark).value = record.remark
  })
